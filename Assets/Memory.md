...
- EnemyBase 만 옛 구간식(100=25% 경감, 0.25/0.125/...)이 남아 있었음 -> def * 0.05f 단일식으로 통일
  - 이제 플레이어/적 방어 공식 동일. 방어력 100=5%, 300=15% 경감 검산 확인
- 달란 지급 패치(GrantGold 등)는 그대로 유지됨

## 달란(골드) 몬스터 처치 지급 (2026-06-14)
- 기획 확인: 달란 획득 2가지 = 몬스터 처치 드롭 + 아이템 판매. 코드엔 판매(ShopSystem.ConfirmSell)만 있었고 처치 드롭 누락
- 재화 이름은 '달란' (코드 gold = 달란, ShopSystem 이미 '달란' 표시)
- 이번 학기: 즉시 지급 (사망 시 PlayerStats.gold 바로 증가). 다음 학기 월드 드롭&획득으로 전환 예정
- 골드 상한 999999 확정 — PlayerStats.gold setter 를 Mathf.Clamp(value, 0, MAX_GOLD) 로 (기존 Mathf.Max(0) 에서 변경), MAX_GOLD 상수 추가
- EnemyBase: goldDropChance(0~1)/goldMin/goldMax 인스펙터 필드, Die 에서 GrantGold 호출
  - 층/종류별 차등은 프리팹마다 값을 다르게 설정 (A 방식). 엑셀 '각인 소비 및 획득 확률'의 층별 재화 개수/확률 참고
  - goldMax<=0 이면 미지급 (기본값이라 기존 프리팹은 설정 전까지 달란 안 줌 — 프리팹에 수치 입력 필요)
- B 전환 대비: 지급 로직을 GrantGold() 메서드로 분리. 다음 학기 데이터 테이블 기반 전환 시 이 메서드 내부만 교체
- 제외(다음 학기): 유물 '드랍 달란 증가' 보너스 연동, 월드 드롭 줍기
